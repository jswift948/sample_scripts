...
$NFS_SERVER="wppnas05.$DOMAIN";

# Set IMPORT_DIR to the directory that conatins the dfsutil import data
$IMPORT_DIR="\\\\$NFS_SERVER/home_2/home/jsswift/dfs";
...
$LOG_FILE="C:/temp/wpb_dfsutil_update_log.txt";
$MIN_LINES="975";
...
open(MOUNT,"C:/SFU/common/mount.exe|") or die "Unable to use mount, $!\n";
...
while ( "$CHANGE_REMOVE" eq "YES" or "$CHANGE_ADD" eq "YES" ) {
	...
	ADD_LINK: foreach $MNT_POINT(keys(%AUTOMOUNT_TABLES)) {
		...
		if ( $AUTOMOUNT_TABLES{"$MNT_POINT"} ne $CURRENT_DFS_TABLES{"$MNT_POINT"} and
			...
			#print "system \"dfscmd /map $DFSROOT\\$MNT_POINT $MNT_PATH1 $COMMENT\"\n";
	
			system("dfscmd /map $DFSROOT\\$MNT_POINT $MNT_PATH1 $COMMENT");
			if ( $MNT_PATH2 ne "" ) { 
